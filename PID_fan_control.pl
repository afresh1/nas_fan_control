...
## in this script.
my $config_file = '/root/nas_fan_control/PID_fan_control_config.ini';

##DEFAULT VALUES
## Use the values declared below if the config file is not present
my $hd_ave_target = 38;         # PID control loop will target this average temperature for the warmest N disks
my $hd_ave_temp_old = 0;        # PID control loop something
...
my $log_temp_summary_only      = 0; # 1 if not logging individual HD temperatures.  0 if logging temp of each HD
my $log_header_hourly_interval = 2; # number of hours between log headers.  Valid options are 1, 2, 3, 4, 6 & 12.
...
my $hd_max_allowed_temp = 40; # celsius. PID control aborts and fans set to 100% duty cycle when a HD hits this temp.
...
my $hd_fans_cool_cpu = 1;      # 1 if the hd fans should spin up to cool the cpu, 0 otherwise

## HD FAN DUTY CYCLE TO OVERRIDE CPU FANS
my $cpu_fans_cool_hd            = 1;  # 1 if the CPU fans should spin up to cool the HDs, when needed.  0 otherwise.  This may be
...
my $hd_cpu_override_duty_cycle = 95;  # when the HD duty cycle equals or exceeds this value, the CPU fans may be overridden to help cool HDs

## CPU TEMP CONTROL
my $cpu_temp_control = 1;  # 1 if the script will control a CPU fan to control CPU temperatures.  0 if the script only controls HD fans.
...
my $cpu_max_fan_speed    = 1800;
my $hd_max_fan_speed     = 3300;
...
#my $hd_fan_duty_start    =  60;    # HD fan duty cycle when script starts - defined in config file
...
my $cpu_fan_header = "FAN2";                 # used for CPU Fan monitoring  and forprinting to standard output for debugging
my $hd_fan_header  = "FANB";                 # used for HD Fan monitoring and printing to standard output for debugging
my @hd_fan_list = ("FANA", "FANB", "FANC");  # used for logging to file
...
my $fan_unreadable_time        = 0;        # the time when a fan read failure started, 0 if there is none.
my $bmc_fail_count             = 0;        # how many times the fans failed verification in the last period.

#this is the last cpu temp that was read
my $last_cpu_temp = 0;

# Whether we are overriding the cpu fan to HIGH
my $cpu_fan_override = 0;

# These should come when loading the config file.
my $hd_duty = $hd_fan_duty_start;
my $default_hd_fan_duty_start = $hd_fan_duty_start;
my $default_hd_ave_target = $hd_ave_target;
my $default_hd_num_peak   = $hd_num_peak;
our $config_num_disks;
our $config_Ta;
our $config_Kp; my $default_Kp = $Kp;
our $config_Ki; my $default_Ki = $Ki;
our $config_Kd; my $default_Kd = $Kd;
our $config_hd_fan_start;
...
sub main
{
    ...
    my $next_log_hour = ( int( $hour/$log_header_hourly_interval ) + 1 ) * $log_header_hourly_interval;
    my $next_log_time;
    ...
    my ($hd_min_temp, $hd_max_temp, @hd_temps) = get_hd_temps();
    $hd_ave_temp_old = shift @hd_temps;
    my ($hd_ave_target, $Kp, $Ki, $Kd, $hd_num_peak, $hd_fan_duty_start, $config_time) = read_config();
    ...
    while()
    {
        ...
        if( $check_time - $last_hd_check_time >= $hd_polling_interval )
        {
            ...
            my $hd_fan_duty_old = $hd_fan_duty;
